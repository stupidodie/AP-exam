...
fresh(Capcity) ->
  gen_server:start(?MODULE, Capcity, []).
          ...
          case searchItem(List, Key) of
            true ->
              ...
              {reply,
               ...
               {OriginalCapcity,
                CurrentCapcity - C + C1,
                [{Key, Value, C, false, [], false, [], []} | NewList]}};
            false ->
              {reply,
               ok,
               {OriginalCapcity,
                CurrentCapcity - C,
                [{Key, Value, C, false, [], false, [], []} | List]}}
          end;
        false ->
          {NewCapcity, NewList} = releaseItem(C, CurrentCapcity, List),
          {reply,
           ok,
           {OriginalCapcity, NewCapcity - C, [{Key, Value, C, false, [], false, [], []} | NewList]}}
      end;
    false ->
      ...
      case C1 == 0 of
        false ->
          ...
          {reply,
           ...
           {OriginalCapcity,
            CurrentCapcity,
            [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | NewList]}};
        true ->
          {reply, nothing, {OriginalCapcity, CurrentCapcity, List}}
      end;
    false ->
      {reply, nothing, {OriginalCapcity, CurrentCapcity, List}}
  end;
        ...
        false ->
          case C1 == 0 of
            false ->
              case Judge2 of
                false ->
                  {reply, {error, alreadyExists}, {OriginalCapcity, CurrentCapcity, List}};
                true ->
                  NewList = addRecovery(Key, From, {insert, Key, Value, C}, List),
                      ...
                      case C =< CurrentCapcity + C1 of
                        true ->
                          {reply,
                           ...
                           {OriginalCapcity,
                            CurrentCapcity + C1 - C,
                            [{Key, Value, C, false, [], false, [], []} | ListAfterDelete]}};
                        false ->
                          {NewCapcity, NewList} =
                            releaseItem(C, CurrentCapcity + C1, ListAfterDelete),
                          {reply,
                           ok,
                           {OriginalCapcity,
                            NewCapcity - C,
...
handle_call({upsert, Key, Fun}, From, {OriginalCapcity, CurrentCapcity, List}) ->
  case searchItem(List, Key) of
    true ->
      {_, Value1, _, Judge1, _, Judge2, _, _} = findItem(List, Key),
      case Judge1 == false of
        true ->
          case Judge2 of
            false ->
              handle_upsert_fun(Fun, {existing, Value1}, Key, From, self()),
              {noreply, {OriginalCapcity, CurrentCapcity, setJudge1(Key, List, true)}};
            true ->
              NewList = addRecovery(Key, From, {upsert, Key, Fun}, List),
              {noreply, {OriginalCapcity, CurrentCapcity, NewList}}
          end;
        false ->
          ...
          case (Judge1 == false) and (Judge2 == false) of
            true ->
              ...
              {reply,
               ...
               {OriginalCapcity,
                CurrentCapcity,
                [{Key, Val, C, Judge1, W1, Judge2, R1, S1} | NewList]}};
            false ->
              {noreply, addStable(Key, From, {Key, Ref}, List)}
          end;
        true ->
  ...
  case Judge1 of
    true ->
      {noreply,
       {OriginalCapcity,
        CurrentCapcity,
        setJudge2(Key, setJudge1(Key, List, Option1), Option2)}};
    false ->
      {noreply,
      ...
      case Judge1 of
        false ->
          case R1 == [] of
            ...
            true ->
              NewList = deleteItem(List, Key),
              lists:foreach(fun({From, {_, Ref}}) -> gen_server:reply(From, {Ref, Value}) end, S1),
              {noreply,
               {OriginalCapcity,
                CurrentCapcity,
                setJudge2(Key, [{Key, Value, C, Judge1, W1, Judge2, R1, []} | NewList], Option)}}
          end;
        true ->
          {noreply,
           {OriginalCapcity,
            CurrentCapcity,
            setJudge1(Key, [{Key, Value, C, false, W1, Judge2, R1, S1} | List], false)}}
      end;
    false ->
                  ...
                  case C =< CurrentCapcity + C1 of
                    true ->
                      ...
                      {noreply,
                       {OriginalCapcity,
                        CurrentCapcity + C1 - C,
                        [{Key, Val, C, J1, W1, J2, R1, S1} | ListAfterDelete]}};
                    false ->
...
addRecovery(Key, From, Operation, List) ->
  case List of
    ...
    [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | Rest] ->
      case Key1 == Key of
        true ->
          [{Key1, Value1, C1, Judge1, Wait1, Judge2, [{From, Operation} | R1], S1} | Rest];
        false ->
          [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | addRecovery(Key,
                                                                           From,
                                                                           Operation,
                                                                           Rest)]
      end
...
setJudge1(Key, List, Option) ->
  case List of
    [] ->
      [];
    [{Key1, V1, C1, Judge, W1, Judge2, R1, S1} | Rest] ->
      case Key1 == Key of
        true ->
          [{Key1, V1, C1, Option, W1, Judge2, R1, S1} | Rest];
        false ->
          [{Key1, V1, C1, Judge, W1, Judge2, R1, S1} | setJudge1(Key, Rest, Option)]
      end
  end.
...
setJudge2(Key, List, Option) ->
  case List of
    [] ->
      [];
    [{Key1, V1, C1, Judge1, W1, Judge2, R1, S1} | Rest] ->
      case Key1 == Key of
        true ->
          [{Key1, V1, C1, Judge1, W1, Option, R1, S1} | Rest];
        false ->
          [{Key1, V1, C1, Judge1, W1, Judge2, R1, S1} | setJudge1(Key, Rest, Option)]
      end
  end.
...
addWait(Key, From, Operation, List) ->
  case List of
    ...
    [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | Rest] ->
      case Key1 == Key of
        true ->
          [{Key1, Value1, C1, Judge1, [{From, Operation} | Wait1], Judge2, R1, S1} | Rest];
        false ->
          [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | addWait(Key, From, Operation, Rest)]
      end
  end.
...
addStable(Key, From, Operation, List) ->
  case List of
    ...
    [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | Rest] ->
      case Key1 == Key of
        true ->
          [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, [{From, Operation} | S1]} | Rest];
        false ->
          [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | addStable(Key,
                                                                         From,
                                                                         Operation,
                                                                         Rest)]
      end
...
sendRemovedMessage(Wait) ->
  case Wait of
    [] ->
      [];
    [{From, _} | Rest] ->
      gen_server:reply(From, {error, removed}),
      sendRemovedMessage(Rest)
  end.
    ...
    [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | Rest] ->
      case Key1 =:= Key of
        true ->
          Rest;
        false ->
          [{Key1, Value1, C1, Judge1, Wait1, Judge2, R1, S1} | deleteItem(Rest, Key)]
      end
  end.
